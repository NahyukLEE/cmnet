...
class FurthestSampling(Function):
    ...
    def forward(ctx, xyz, offset, new_offset):
        ...
        xyz = xyz.to(torch.float32) # ADDED for equivariance checking
...
class KNNQuery(Function):
    ...
    def forward(ctx, nsample, xyz, new_xyz, offset, new_offset):
        ...
        xyz = xyz.to(torch.float32) # ADDED for equivariance checking
        new_xyz = new_xyz.to(torch.float32) # ADDED for equivariance checking
...
def queryandgroup(nsample, xyz, new_xyz, feat, idx, offset, new_offset, use_xyz=True):
    ...
    n, m, c = xyz.shape[0], new_xyz.shape[0], feat.shape[0]
    grouped_xyz = xyz[idx.view(-1).long(), :].view(m, nsample, 3) # (m, nsample, 3)
    # grouped_xyz = grouping(xyz, idx) # (m, nsample, 3)
    grouped_xyz -= new_xyz.unsqueeze(1) # (m, nsample, 3)
    grouped_feat = feat[:, :, idx.view(-1).long()].view(c,3,m,nsample).permute(2,3,0,1)
    # ERROR: grouped_fett = feat[:, :, idx.view(-1).long()].view(m, nsample, c, 3) # (m, nsample, c)
...
def interpolation(xyz, new_xyz, feat, offset, new_offset, k=3):
    ...
    new_feat = torch.cuda.FloatTensor(feat.shape[0], 3, new_xyz.shape[0]).zero_()
    for i in range(k):
