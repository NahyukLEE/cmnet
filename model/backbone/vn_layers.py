...
class VNInstanceNorm(nn.Module):
    def __init__(self, num_features, dim):
        super(VNInstanceNorm, self).__init__()
        self.dim = dim
        ...
        if dim == 3 or dim == 4:
            ...
            self.inst_norm = nn.InstanceNorm2d(num_features, affine=True)
    
    def forward(self, x):
        '''
        x: point features of shape [B, N_feat, 3, N_samples, ...]
        '''
        norm = torch.norm(x, dim=2) + self.eps  # Shape: [B, N_feat, N_samples, ...]
        norm_in = self.inst_norm(norm)
        norm = norm.unsqueeze(2)  # Shape: [B, N_feat, 1, N_samples, ...]
        norm_in = norm_in.unsqueeze(2)  # Shape: [B, N_feat, 1, N_samples, ...]
        x = x / norm * norm_in
